...
int main(int ac, char* av[]) 
{
	...
	StateMonitor * zi_mon = new StateMonitor(bcpnn_con->tr_z_pre,5,sys->fn("zi","txt"));
	StateMonitor * zj_mon = new StateMonitor(bcpnn_con->tr_z_post,5,sys->fn("zj","txt"));
	StateMonitor * pi_mon = new StateMonitor(bcpnn_con->tr_p_pre,5,sys->fn("pi","txt"));
	StateMonitor * pj_mon = new StateMonitor(bcpnn_con->tr_p_post,5,sys->fn("pj","txt"));

	StateMonitor * bias_mon = new StateMonitor(poneurons->get_state_vector("w"),5,sys->fn("bj","txt"));

	// // Record spikes
   	SpikeMonitor * smon_pr = new SpikeMonitor(prneurons,sys->fn("prspikes","txt"));
	SpikeMonitor * smon_po = new SpikeMonitor(poneurons,sys->fn("pospikes","txt"));

	VoltageMonitor * vmon_pr = new VoltageMonitor(prneurons,5,sys->fn("vmem_pr","txt"));
	VoltageMonitor * vmon_po = new VoltageMonitor(poneurons,5,sys->fn("vmem_po","txt"));
	
    // // Record input firing rate (sample every 1s)
	PopulationRateMonitor * pmon_in = new PopulationRateMonitor(prneurons,sys->fn("prrate","txt"), 0.1 );
	
    // // Record output firing rate (sample every 1s)
	PopulationRateMonitor * pmon_ut = new PopulationRateMonitor(poneurons,sys->fn("porate","txt"), 0.1 );
	
	// Record individual synaptic weights (sample every 10 ms)
	WeightMonitor * pijmon = new WeightMonitor(bcpnn_con,5,5,sys->fn("pij","txt"),0.01,SINGLE,1);
	WeightMonitor * wijmon = new WeightMonitor(bcpnn_con,5,5,sys->fn("wij","txt"),0.01,SINGLE,0);
